...
final class PlgSystemGrit_competitor_tab extends CMSPlugin
{
    ...
    public function onAfterRender(): void
    {
        ...
        $script = '<script>(function(){'
            ...
            . 'function init(){var tc=document.querySelector(".tab-content");if(!tc){return;} if(!document.getElementById("grit-competitor-tab-pane")){var p=document.createElement("div");p.className="tab-pane";p.id="grit-competitor-tab-pane";p.innerHTML=paneHtml;tc.appendChild(p);} var f=document.getElementById("grit-competitor-form");if(f&&!f.dataset.binded){f.dataset.binded="1";var saveBtn=document.getElementById("grit-competitor-save");if(saveBtn){saveBtn.addEventListener("click",function(){var fd=new FormData();fd.append("action","save");fd.append("id",f.querySelector("input[name=grit_cp_id]").value||"");fd.append("product_id",f.querySelector("input[name=grit_cp_product_id]").value||pid);fd.append("competitor_name",f.querySelector("input[name=grit_cp_competitor_name]").value||"");fd.append("url",f.querySelector("input[name=grit_cp_url]").value||"");fd.append("selector",f.querySelector("input[name=grit_cp_selector]").value||"");fd.append("price",f.querySelector("input[name=grit_cp_price]").value||"");fetch(ajaxUrl,{method:"POST",body:fd,credentials:"same-origin"}).then(function(r){return r.json();}).then(function(d){var r=document.getElementById("grit-competitor-result");if(d&&d.success){r.innerHTML="<span style=\\"color:green\\">Сохранено</span>";Array.prototype.forEach.call(f.querySelectorAll("input[name=grit_cp_competitor_name],input[name=grit_cp_url],input[name=grit_cp_selector],input[name=grit_cp_price]"),function(inp){inp.value="";});f.querySelector("input[name=grit_cp_product_id]").value=pid;f.querySelector("input[name=grit_cp_id]").value="";loadList();}else{r.innerHTML="<span style=\\"color:#a00\\">Ошибка сохранения</span>";}}).catch(function(){var r=document.getElementById("grit-competitor-result");r.innerHTML="<span style=\\"color:#a00\\">Ошибка запроса</span>";});});}} loadList();}'
