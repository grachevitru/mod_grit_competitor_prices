...
final class PlgSystemGrit_competitor_tab extends CMSPlugin
{
    ...
    public function onAfterRender(): void
    {
        ...
        $script = '<script>(function(){'
            ...
            . 'function bindActions(items,f){var listEl=document.getElementById("grit-competitor-list");if(!listEl||!f){return;} Array.prototype.forEach.call(listEl.querySelectorAll(".grit-edit"),function(btn){btn.addEventListener("click",function(){var id=this.getAttribute("data-id");var row=(items||[]).find(function(x){return String(val(x,"id","ID",0))===String(id);});if(!row){return;}f.querySelector("input[name=id]").value=val(row,"id","ID",0)||"";f.querySelector("input[name=competitor_name]").value=val(row,"competitor_name","COMPETITOR_NAME",1)||"";f.querySelector("input[name=url]").value=val(row,"url","URL",2)||"";f.querySelector("input[name=selector]").value=val(row,"selector","SELECTOR",3)||"";f.querySelector("input[name=price]").value=val(row,"price","PRICE",4)||"";window.scrollTo({top:listEl.offsetTop-120,behavior:"smooth"});});}); Array.prototype.forEach.call(listEl.querySelectorAll(".grit-del"),function(btn){btn.addEventListener("click",function(){var id=this.getAttribute("data-id");if(!confirm("Удалить запись?")){return;}var fd=new FormData();fd.append("action","delete");fd.append("id",id);fd.append("product_id",pid);fetch(ajaxUrl,{method:"POST",body:fd,credentials:"same-origin"}).then(function(r){return r.json();}).then(function(){loadList();}).catch(function(){});});});}'
